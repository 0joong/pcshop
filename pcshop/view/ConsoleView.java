...
public class ConsoleView {
	...
	public int selectParts(PartStorage partStorage, String parts) {
		...
		int partsCode;
		do{
		partsCode = readNumber(">> " + parts + " PRODUCTCODE 입력 : ");
		
		if(!partStorage.isValidPart(partsCode, parts)){
			System.out.println("올바르지 않은 제품번호입니다.");
		}
		}while(!partStorage.isValidPart(partsCode));

		return partsCode;
	}


	public int selectFinishedPC(PCStorage pcStorage) {
		int productCode;
		do{
			productCode = readNumber(">> " + "PC" + " PRODUCTCODE 입력 : ");

			if(!pcStorage.isValidPC(productCode)){
				System.out.println("올바르지 않은 제품번호입니다.");
			}
		}while(!pcStorage.isValidPC(productCode));
